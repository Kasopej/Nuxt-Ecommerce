<script setup>
  ...
  const loaderDebounced = refDebounced(loader, 1000);

  watch(
    filters,
    (value) => {
      loader.value = true;
      productStore.fetchProducts().then(() => (loader.value = false));
...
<template>
  <div class="filters-wrapper flex gap-2 items-center">
    <app-spinner v-if="loaderDebounced" class="mt-10"></app-spinner>
